...
const FACE_OVERRIDES_JAVA = {
    ...
    'campfire':    { top: 'campfire_log_lit', bottom: 'campfire_log_lit', side: 'oak_log' },
    'soul_campfire': { top: 'soul_campfire_log_lit', bottom: 'soul_campfire_log_lit', side: 'oak_log' },
};
...
export function getFaceUrls(blockId, options = {}) {
    ...
    if (local === 'campfire' || local === 'soul_campfire') {
        // 候補名を優先順に試す（Java/Bedrock 両方のリソースパック命名に対応）
        const logNames  = ['oak_log', 'log_oak', 'log'];
        const topNames  = local === 'campfire'
            ? ['campfire_log_lit', 'campfire_log', 'oak_log', 'log']
            : ['soul_campfire_log_lit', 'soul_campfire_log', 'oak_log', 'log'];
        const logUrl = logNames.map(n => _state.textures.get(n)).find(Boolean) || null;
        const topUrl = topNames.map(n => _state.textures.get(n)).find(Boolean) || logUrl;
        if (logUrl || topUrl) {
            const s = { url: logUrl || topUrl, tint: null };
            const t = { url: topUrl || logUrl, tint: null };
            return { east: s, west: s, top: t, bottom: t, north: s, south: s, found: true };
        }
    }
    ...
    let variantIdx = _resolveVariantIndex(effectiveStates);
    ...
    if (_state.isBedrock) {
        ...
        if (expanded) {
            ...
            if (top || side) {
                ...
                // Force correct textures for campfires (both Java and Bedrock paths)
                if (local === 'campfire' || local === 'soul_campfire') {
                    const logTex = local === 'campfire' ? 'campfire_log_lit' : 'soul_campfire_log_lit';
                    const topFace = apply(toFaceObj(_resolveTextureKey(logTex, useVariantIdx) || top));
                    const sideFace = apply(toFaceObj(_resolveTextureKey('oak_log', useVariantIdx) || side));
                    return {
                        east: sideFace, west: sideFace,
                        top: topFace, bottom: topFace,
                        north: sideFace, south: sideFace,
                        found: true,
                    };
                }
