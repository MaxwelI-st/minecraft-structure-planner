...
async function handleConvertLitematic(taskId, payload) {
  ...
  for (const tag of reader.walkCompound(bodyOffset)) {
    ...
    if (tag.name === 'structure') {
      sendProgress(taskId, 0.1, 'Reading block indices…');
      const parsed = _parseStructureForConversion(reader, tag.value.offset);
      bedrockPalette = parsed.palette;
      layer0Indices  = parsed.layer0;
      layer1Indices  = parsed.layer1;
    }
  }

  if (!dimensions || !layer0Indices) {
  ...
  // ── Step 2: Load mapping Map from mapping.worker.js ─────────────────────
  // For the first conversion, we request the mapping via a BroadcastChannel
  // or (simpler for Phase 2) fetch the JSON directly in this Worker.
  ...
  if (hasWaterlogging) {
    ...
    // Simple remap: bedrockIdx → jeIdx for every block
  ...
  javaIndices = null; // Release ZYX buffer for GC
...
async function _loadMappingMap() {
  ...
  try {
    ...
    console.warn('process.worker: mapping JSON unavailable —', err.message);
    _cachedMappingMap = new Map(); // Empty map → converter will use stone fallback
  }
...
async function _gzip(buffer) {
  if (typeof CompressionStream !== 'undefined') {
    ...
    // Fallback: dynamically import pako (must be bundled by Vite)
...
async function handleInvertGen(taskId, payload) {
  ...
  sendComplete(taskId, buf2, { mode, dimensions, ...result.stats });
}
